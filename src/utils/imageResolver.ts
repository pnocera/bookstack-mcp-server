...
function classifyInternalIp(ip: string): string | null {
  if (ip === '127.0.0.1' || ip === '::1' || ip === '0:0:0:0:0:0:0:1') return 'loopback';
...
async function validateNotInternalHost(hostname: string): Promise<void> {
  ...
  for (const { address } of addresses) {
    ...
    if (reason) {
      throw new Error(
        `URL resolves to a ${reason} IP address (${address}) — only public URLs are allowed`
      );
    }
  }
}
...
async function fetchImageFromUrl(url: string, fallbackName: string): Promise<PreparedImage> {
  let currentUrl = url;
  ...
  for (let hop = 0; hop <= MAX_REDIRECTS; hop++) {
    ...
    let response;
    try {
      response = await axios.get<ArrayBuffer>(currentUrl, {
    ...
    if (!ALLOWED_MIME_TYPES.has(rawMime)) {
      throw new Error(
        `Unsupported image MIME type: "${rawMime || '(none)'}". Allowed: ${[...ALLOWED_MIME_TYPES].sort().join(', ')}`
      );
    }
...
export async function resolveImage(image: string, fallbackName: string): Promise<PreparedImage> {
